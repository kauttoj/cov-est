...
DATA = DATA(:,:,:,randsample(size(DATA,4),CELLS));
DATA = DATA + 0*0.00000001*rand(size(DATA));
...
GRID = {logspace(-4.5,-0.3,15),logspace(-4.5,-0.3,15)};
...
[hypers, bestDelta, visited, losses, quality_sample] = cove.crossEstimateHyper(DATA,size(DATA,1),'sample');%{10.^(-linspace(0.2,5,15)),10.^(-linspace(0.05,5,25))});
...
[hypers, bestDelta, visited, losses, quality_diag] = cove.crossEstimateHyper(DATA,size(DATA,1),'diag',GRID);%{10.^(-linspace(0.2,5,15)),10.^(-linspace(0.05,5,25))});
[cormat_diag,M_diag, V_diag, extras_diag,covmat_diag] = cove.estimate(DATA,size(DATA,1),'diag',hypers); % alpha = 0.05; beta = 0.25
subplot(1,5,5);imagesc(nodiag(cormat_diag));axis image;colorbar;
title(sprintf('yatsenko diag (%f)',corr(real_corrmat(IND),cormat_diag(IND))));

[hypers, bestDelta, visited, losses, quality_glasso] = cove.crossEstimateHyper(DATA,size(DATA,1),'glasso',GRID(1));%{10.^(-linspace(0.2,5,15)),10.^(-linspace(0.05,5,25))});
[cormat_glasso,M_glasso,V_glasso,extras_glasso,covmat_glasso] = cove.estimate(DATA,size(DATA,1),'glasso',hypers); % alpha = 0.05; beta = 0.25
subplot(1,5,3);imagesc(nodiag(cormat_glasso));axis image;colorbar;
title(sprintf('yatsenko glasso (%f)',corr(real_corrmat(IND),cormat_glasso(IND))));

[hypers, bestDelta, visited, losses, quality_lvglasso] = cove.crossEstimateHyper(DATA,size(DATA,1),'lv-glasso',GRID);%{10.^(-linspace(0.2,5,15)),10.^(-linspace(0.05,5,25))});
[cormat_lvglasso,M_lvglasso,V_lvglasso,extras_lvglasso,covmat_lvglasso] = cove.estimate(DATA,size(DATA,1),'lv-glasso',hypers); % alpha = 0.05; beta = 0.25
subplot(1,5,4);imagesc(nodiag(cormat_lvglasso));axis image;colorbar;
